...
int32_t rc32::rc32_read(FILE* file, char *buf, int32_t lenght){
  ...
  if(init){
    if((bufsize=flt->read_flt(file,(char*)pqbuffer,_RC32_BUFFER_SIZE))<0)return -1;
    if((bufsize==0)&&flt->is_eof(file)) finalize=true;
...
int32_t rc32::rc32_write(FILE* file, char *buf, int32_t lenght){
  ...
  if((lenght==0)&&finalize){
    if(bufsize&&(flt->write_flt(file,(char*)pqbuffer,bufsize)<0)) return -1;
    bufsize=0;
    for(int i=sizeof(uint32_t);i>0;i--){
      pqbuffer[bufsize++]=(uint8_t)(low>>24);
      low<<=8;
    }
    if(flt->write_flt(file,(char*)pqbuffer,bufsize)<0) return -1;
    eof=true;
  }
  else while(lenght--){
    uint16_t i;
